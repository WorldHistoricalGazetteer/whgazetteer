...
def align_whg(pk, *args, **kwargs):
  ...
  # utility validates, selects pass1 hit
  def selectHit(hits):
    # 
    print(hits)
  ...
  for place in qs:
    ...
    if result_obj['hit_count'] == 0:
      ...
      try:
        ...
        place.indexed = True
        place.save()
        print('new parent: '+whg_id+' from place: '+place.id+' ('+place.title+')')
      except:
      ...
      # deal with pass1 (shared link)
      pass1hits = [hit for hit in result_obj['hits'] if hit['pass']=='pass1']
      if len(pass1hits) == 0:
        ...
        # index as child if hit is parent, else as sibling (child of hit's parent)
        hit = result_obj['hits'][0]
      elif len(pass1hits) > 1:
        # choose one 
        hit = selectHit(pass1hits)
      count_p1+=1  
      # prep a doc
      newchild = makeDoc(place, 'none')      
      # is hit with shared link a parent or child?
      role = hit['_source']['relation']['name']
      # get correct parent _id (whg_id)
      if role == 'child':
