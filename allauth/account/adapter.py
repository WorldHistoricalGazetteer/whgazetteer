...
class DefaultAccountAdapter(object):
    ...
    for ext in ["html", "txt"]:
      try:
        ...
        if ext == "txt" and not bodies:
          ...
          raise
    #print('render_mail() bodies', bodies)
    if "txt" in bodies:
  ...
  # template_prefix = 'account/email/password_reset_key' # bad, needs accounts/password/reset/key/
  # settings.URL_FRONT = 'http://localhost:8000/' or 'http://whgazetteer.org/'
  # server version needs to replace 'localhost' domain prefix
  
  # context from SignupView()
  # {
  # 'user': <User: fubar>, 
  # 'activate_url': 'http://localhost:8000/accounts/confirm-email/MTg4:1lFNk6:MbyefKKYEGswDnqleoWSgLk5rQE/', 
  # 'current_site': <Site: http://whgazetteer.org>, 
  # 'key': 'MTg4:1lFNk6:MbyefKKYEGswDnqleoWSgLk5rQE'}
  
  # context from PasswordResetView() 
  # {
  # 'current_site': <Site: http://whgazetteer.org>, 
  # 'user': <User: fubar>, 
  # 'password_reset_url': 'http://localhost:8000/accounts/password/reset/key/7m-5og-f5105b88a4d10b7a6d1c/', 
  ...
  # modified 13 Nv 2021 to catch reset password case
  def send_mail(self, template_prefix, email, context):
    print('context in send_mail()', context)
    print('URL_FRONT', settings.URL_FRONT)
    try:
        if 'password_reset_url' in context:
          context['password_reset_url'] = context['password_reset_url'].replace(
              'http://localhost:8000/', settings.URL_FRONT)
          msg = self.render_mail(template_prefix, email, context)
        else:
          context['activate_url'] = settings.URL_FRONT + \
              'accounts/confirm-email/' + context['key']
